...
class JobScreeningAgent:
    ...
    def move_to_next_question(self):
        ...
        if self.current_question < len(self.questions) - 1:
            ...
            return True  # Stop the loop if all questions have been answered

    def run(self):
        # Collect responses
        while True:
            question = self.get_question()
            if question == "Thank you for answering all the questions!":
                break

            print(f"Question: {question}")
            answer = input("Your answer: ")  # Simulate user input
            self.responses.append(answer)

            follow_up = self.get_follow_up()
            if follow_up:
                print(f"Follow-up: {follow_up}")
                follow_up_answer = input("Your answer: ")  # Simulate user input
                self.responses.append(follow_up_answer)

            if self.move_to_next_question():  # Check if all questions have been answered
                break  # Exit loop if no more questions

        # After the questions are completed, print the responses for debugging
        print("Responses:", self.responses)

        # Pass responses to LLM for evaluation
        evaluation_result = self.evaluate_with_llm(self.responses)
    ...
    # Build a prompt to evaluate responses based on job-specific criteria
        ...
        Provide a score (out of 10) for the overall score, qualifications, enthusiasm, and availability.
...
# Example: Start the conversation with a user applying for the "Shop Helper/Laborer" role
job_title = "Shop Helper/Laborer"  # This would come from user input
agent = JobScreeningAgent(job_title)
agent.run()
